...
namespace TestTask
{
    public class Program
    {
        ...
        private static IList<LetterStats> FillSingleLetterStats(IReadOnlyStream stream)
        {
            ...
            var letters = new List<LetterStats>();
            while (!stream.IsEof)
            {
                char c = stream.ReadNextChar();

                // TODO : заполнять статистику с использованием метода IncStatistic. Учёт букв - регистрозависимый.
                var currentIndex = letters.FindIndex(x => x.Letter == c.ToString());
                if (currentIndex == -1)
                {
                    letters.Add(new LetterStats { Letter = c.ToString(), Count = 1 });
                }
                else
                {
                    var currentLetter = letters[currentIndex];
                    IncStatistic(ref currentLetter);
        ...
        private static IList<LetterStats> FillDoubleLetterStats(IReadOnlyStream stream)
        {
            ...
            {
                char c = stream.ReadNextChar();
                if (prevLetter == (char)0)
                {
                    prevLetter = c.ToString().ToUpper()[0];
                    continue;
                }
                if (prevLetter == c.ToString().ToUpper()[0])
                {
                    var currentIndex = letters.FindIndex(x => x.Letter == new string(prevLetter, 2));
                    if (currentIndex == -1)
                    {
                        letters.Add(new LetterStats { Letter = new string(prevLetter, 2), Count = 1 });
                    }
                    else
                    {
                        var currentLetter = letters[currentIndex];
        ...
        private static void RemoveCharStatsByType(ref IList<LetterStats> letters, CharType charType)
        {
            ...
            switch (charType)
            {
                case CharType.Consonants:
                    filter = "BCDFGHJKLMNPQRSTVWXYZ";
                    break;
                case CharType.Vowel:
                    filter = "AEIOUY";
                    break;
            }
            letters = letters.Where(x => !filter.Contains(x.Letter[0].ToString().ToUpper())).ToList();
